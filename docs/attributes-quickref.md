...
| `wf-algolia-base-numeric-filter` | (1.0.14) Always-on Algolia `numericFilters` for the browse section or static list, comma-separated: `"createdDate >= now-30d, points > 100"`. Ops `>=` `<=` `!=` `>` `<` `=`; value is a number or `now[±N{s\|m\|h\|d\|w}]`, resolved at query time. Merges with UI numeric filters in single-index, disjunctive, and federated browse queries. Invisible to filter UI/URL-sync. Malformed expressions warn once and are ignored; valid expressions still apply. |
| `wf-algolia-base-numeric-filter-unit="s"` | (1.0.14) `now` tokens resolve to epoch **milliseconds** by default (Xano-native); set `"s"` for indexes that store epoch seconds. |
...
| `wf-algolia-srcset` | Responsive image candidates; see [Responsive images](../README.md#responsive-images) for field fallback, Xano auto mode, and opt-out behavior. |
